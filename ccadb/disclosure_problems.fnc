...
BEGIN
	...
	IF t_disclosureStatus = 'DisclosureIncomplete' THEN
		IF coalesce(coalesce(t_ccadbCertificate.CP_URL, t_ccadbCertificate.CP_CPS_URL), t_ccadbCertificate.MD_CP_CPS_URL) IS NULL THEN
			t_problems := array_append(t_problems, '"Certificate Policy URL" or "Certificate Practice & Policy Stmt URL" or "MD/AsciiDoc CP/CPS URL" is required');
		END IF;
		IF coalesce(coalesce(t_ccadbCertificate.CP_EFFECTIVE_DATE, t_ccadbCertificate.CP_CPS_EFFECTIVE_DATE), t_ccadbCertificate.MD_CP_CPS_EFFECTIVE_DATE) IS NULL THEN
			t_problems := array_append(t_problems, '"CP Effective Date" or "CP/CPS Effective Date" or "MD/AsciiDoc CP/CPS Effective Date" is required');
		ELSIF coalesce(coalesce(t_ccadbCertificate.CP_EFFECTIVE_DATE, t_ccadbCertificate.CP_CPS_EFFECTIVE_DATE), t_ccadbcertificate.MD_CP_CPS_EFFECTIVE_DATE) < (now() AT TIME ZONE 'UTC' - interval '365 days') THEN
			t_problems := array_append(t_problems, '"CP Effective Date" or "CP/CPS Effective Date" or "MD/AsciiDoc CP/CPS Effective Date" is older than 365 days');
		END IF;
		IF coalesce(coalesce(t_ccadbCertificate.CPS_URL, t_ccadbCertificate.CP_CPS_URL), t_ccadbCertificate.MD_CP_CPS_URL) IS NULL THEN
			t_problems := array_append(t_problems, '"Certification Practice Statement URL" or "Certificate Practice & Policy Stmt URL" or "MD/AsciiDoc CP/CPS URL" is required');
		END IF;
		IF coalesce(coalesce(t_ccadbCertificate.CPS_EFFECTIVE_DATE, t_ccadbCertificate.CP_CPS_EFFECTIVE_DATE), t_ccadbCertificate.MD_CP_CPS_EFFECTIVE_DATE) IS NULL THEN
			t_problems := array_append(t_problems, '"CPS Effective Date" or "CP/CPS Effective Date" or "MD/AsciiDoc CP/CPS Effective Date" is required');
		ELSIF coalesce(coalesce(t_ccadbCertificate.CPS_EFFECTIVE_DATE, t_ccadbCertificate.CP_CPS_EFFECTIVE_DATE), t_ccadbCertificate.MD_CP_CPS_EFFECTIVE_DATE) < (now() AT TIME ZONE 'UTC' - interval '365 days') THEN
			t_problems := array_append(t_problems, '"CPS Effective Date" or "CP/CPS Effective Date" or "MD/AsciiDoc CP/CPS Effective Date" is older than 365 days');
		END IF;
...
/*		IF t_ccadbCertificate.NETSEC_AUDIT_URL IS NULL THEN
		...
		IF trustContextID IN (5, 12) THEN
			IF (nullif(nullif(t_ccadbCertificate.JSON_ARRAY_OF_ALL_FULL_CRL_URLS, ''), '[""]') IS NULL) AND (nullif(nullif(t_ccadbCertificate.JSON_ARRAY_OF_PARTITIONED_CRL_URLS, ''), '[""]') IS NULL) THEN
				SELECT ca.ID, coalesce(ca.NUM_ISSUED[1], 0) + coalesce(ca.NUM_ISSUED[2], 0)
					...
					WHERE cac.CERTIFICATE_ID = certificateID
						AND cac.CA_ID = ca.ID;
				IF t_count = 0 THEN
					t_problems := array_append(t_problems, '"JSON Array of All Full CRL URLs" or "JSON Array of Partitioned CRLs" may be required (<A href="/?ca=' || t_caID || '" target="_blank">no issuance observed</A>)');
				ELSE
					t_problems := array_append(t_problems, '"JSON Array of All Full CRL URLs" or "JSON Array of Partitioned CRLs" is required (<A href="/?ca=' || t_caID || '" target="_blank">' || t_count::text || ' (pre)cert(s) observed</A>)');
				END IF;
			END IF;
			IF nullif(nullif(t_ccadbCertificate.JSON_ARRAY_OF_ALL_FULL_CRL_URLS, ''), '[""]') IS NOT NULL THEN
				FOR l_crl IN (
					SELECT coalesce(crl.ERROR_MESSAGE, 'EXPIRED (nextUpdate = ' || TO_CHAR(crl.NEXT_UPDATE, 'YYYY-MM-DD"T"HH24:MI:SS"Z"') || ')') ERROR_MESSAGE, json_crl_url
						FROM ca_certificate cac, crl, json_array_elements_text(t_ccadbCertificate.JSON_ARRAY_OF_ALL_FULL_CRL_URLS::json) json_crl_url
						WHERE cac.CERTIFICATE_ID = t_ccadbCertificate.CERTIFICATE_ID
							AND cac.CA_ID = crl.CA_ID
							AND length(t_ccadbCertificate.JSON_ARRAY_OF_ALL_FULL_CRL_URLS) > 4	-- Longer than [""].
					...
					t_problems := array_append(t_problems, '"JSON Array of All Full CRL URLs" ERROR: ' || l_crl.json_crl_url || ' => ' || l_crl.ERROR_MESSAGE);
				END LOOP;
			END IF;
			IF nullif(nullif(t_ccadbCertificate.JSON_ARRAY_OF_PARTITIONED_CRL_URLS, ''), '[""]') IS NOT NULL THEN
				FOR l_crl IN (
					SELECT coalesce(crl.ERROR_MESSAGE, 'EXPIRED (nextUpdate = ' || TO_CHAR(crl.NEXT_UPDATE, 'YYYY-MM-DD"T"HH24:MI:SS"Z"') || ')') ERROR_MESSAGE, json_crl_url
						FROM ca_certificate cac, crl, json_array_elements_text(t_ccadbCertificate.JSON_ARRAY_OF_PARTITIONED_CRL_URLS::json) json_crl_url
						WHERE cac.CERTIFICATE_ID = t_ccadbCertificate.CERTIFICATE_ID
							AND cac.CA_ID = crl.CA_ID
							AND length(t_ccadbCertificate.JSON_ARRAY_OF_PARTITIONED_CRL_URLS) > 4	-- Longer than [""].
							AND crl.DISTRIBUTION_POINT_URL = json_crl_url
							AND (
								(crl.ERROR_MESSAGE IS NOT NULL)
								OR (crl.NEXT_UPDATE < now() AT TIME ZONE 'UTC')
...
/*		SELECT min(coalesce(cc2.NETSEC_AUDIT_URL, '&lt;omitted&gt;')), max(coalesce(cc2.NETSEC_AUDIT_URL, '&lt;omitted&gt;')),
	...
	ELSIF t_disclosureStatus = 'DisclosedWithInconsistentCPS' THEN
		SELECT min(coalesce(cc2.CP_URL, '&lt;omitted&gt;')), max(coalesce(cc2.CP_URL, '&lt;omitted&gt;')),
				...
				min(coalesce(cc2.MD_CP_CPS_URL, '&lt;omitted&gt;')), max(coalesce(cc2.MD_CP_CPS_URL, '&lt;omitted&gt;'))
			INTO t_cpUrl1, t_cpUrl2,
				t_cpsUrl1, t_cpsUrl2,
				t_cpCpsUrl1, t_cpCpsUrl2,
				t_mdCpCpsUrl1, t_mdCpCpsUrl2
			FROM ca_certificate cac, ca_certificate cac2, ccadb_certificate cc2
			WHERE cac.CERTIFICATE_ID = certificateID
				AND cac.CA_ID = cac2.CA_ID
				AND EXISTS (
		...
		IF sort_delimited_list(t_cpUrl1, ';') != sort_delimited_list(t_cpUrl2, ';') THEN
			t_problems := array_append(t_problems, 'Certificate Policy URLs: ' || t_cpUrl1 || ' != ' || t_cpUrl2);
		END IF;
		IF sort_delimited_list(t_cpsUrl1, ';') != sort_delimited_list(t_cpsUrl2, ';') THEN
			t_problems := array_append(t_problems, 'Certification Practice Statement URLs: ' || t_cpsUrl1 || ' != ' || t_cpsUrl2);
		END IF;
		IF sort_delimited_list(t_cpCpsUrl1, ';') != sort_delimited_list(t_cpCpsUrl2, ';') THEN
			t_problems := array_append(t_problems, 'Certificate Practice & Policy Stmt URLs: ' || t_cpCpsUrl1 || ' != ' || t_cpCpsUrl2);
		END IF;
		IF sort_delimited_list(t_mdCpCpsUrl1, ';') != sort_delimited_list(t_mdCpCpsUrl2, ';') THEN
			t_problems := array_append(t_problems, 'MD/AsciiDoc CP/CPS URLs: ' || t_mdCpCpsUrl1 || ' != ' || t_mdCpCpsUrl2);
		END IF;
